      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {depoimentos.map((item) => (
            <div
              key={item.nome}
              className="bg-[#F5F8FF] rounded-2xl p-7 flex flex-col gap-4 border border-gray-100"
            >
              <div className="flex gap-1">
                {Array.from({ length: item.estrelas }).map((_, i) => (
                  <svg key={i} viewBox="0 0 24 24" fill="#C9952A" className="w-4 h-4">
              ...
              <div className="flex items-center gap-3 pt-3 border-t border-gray-200">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-xs">{item.perfil}</p>
                </div>
              </div>
            </div>
          ))}
